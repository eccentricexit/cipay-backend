...
export default function buildStarkbankWebhookController(): RequestHandler {
  return async function starkbankWebhookController(
      ...
      try {
        const paymentRequest = await PaymentRequest.findOne({ starkbankPaymentId: id })
        if (!paymentRequest) {
          logger.warn(`StarkbankWebhookController: Starkbank reported on a payment that is not saved on the db. Id: ${id}`)
          return
        }
